...
exports.installApp = function (req, res) {
  ...
  if (typeof req.body.appPath !== "undefined") {
    if (req.body.appPath.slice(-3) === 'apk') {
      if (req.body.appPath.indexOf('http') > -1) {
        helpers.downloadFile(req.body.appPath, ".apk", function (err, downloadedAppPath) {
          if (err) return cb(err);
          install(downloadedAppPath);
        });
      } else {
        install(req.body.appPath);
      }
    } else {
...
exports.replaceApp = function (req, res) {
  var appPath = req.body.appPath; // remote path

  if (checkMissingParams(req, res, {appPath: appPath})) {
    req.device.replaceApp(appPath, req, getResponseHandler(req, res));
  }
};
...
exports.stopApp = function (req, res) {
  ...
  req.device.stopApp(getResponseHandler(req, res), pkg);
};
...
exports.removeFile = function (req, res) {
  ...
  if (checkMissingParams(req, res, {path: path})) {
    req.device.removeFile(path, getResponseHandler(req, res));
  }
};
...
exports.removeFolder = function (req, res) {
  ...
  if (checkMissingParams(req, res, {path: path})) {
    req.device.removeFolder(path, getResponseHandler(req, res));
  }
};
...
exports.listFiles = function (req, res) {
  ...
  if (checkMissingParams(req, res, {path: path})) {
    req.device.listFiles(path, getResponseHandler(req, res));
  }
};
...
exports.listFolder = function (req, res) {
  ...
  if (checkMissingParams(req, res, {path: path})) {
    req.device.listFolder(path, getResponseHandler(req, res));
  }
};

exports.broadcastIntent = function (req, res) {
  var intent = req.body.intent,
      receiver = req.body.receiver,
      pkg = req.body.pkg,
      keys = req.body.keys;

  if (checkMissingParams(req, res, {intent: intent})) {
    req.device.broadcastIntent(intent, pkg, receiver, keys, getResponseHandler(req, res));
  }
};

exports.isSoftKeyboardPresent = function (req, res) {
  req.device.isSoftKeyboardPresent(getResponseHandler(req, res));
};

exports.clearData = function (req, res) {
  var pkg = req.body.pkg;

  req.device.clearData(pkg, getResponseHandler(req, res));
};

exports.swipeUpHomeButton = function (req, res) {
  req.device.swipeUpHomeButton(getResponseHandler(req, res));
};

exports.getNavigationBarRegion = function (req, res) {
  req.device.getNavigationBarRegion(getResponseHandler(req, res));
};

exports.adbSwipe = function (req, res) {
  var x1 = req.body.x1,
      x2 = req.body.x2,
      y1 = req.body.y1,
      y2 = req.body.y2,
  ...
  if (checkMissingParams(req, res, {x1: x1, y1: y1, x2: x2, y2: y2, duration: duration})) {
    req.device.adbSwipe(x1, y1, x2, y2, duration, sleep, getResponseHandler(req, res));
  }
};

exports.adbTap = function (req, res) {
  var x = req.body.x,
      y = req.body.y;
  if (checkMissingParams(req, res, {x: x, y: y})) {
    req.device.adbTap(x, y, getResponseHandler(req, res));
  }
};

exports.getDisplayMetrics = function (req, res) {
  req.device.getDisplayMetrics(getResponseHandler(req, res));
};

exports.hasRoot = function (req, res) {
  req.device.hasRoot(getResponseHandler(req, res));
};

exports.getDate = function (req, res) {
  req.device.getDate(getResponseHandler(req, res));
};

exports.getDateString = function (req, res) {
  req.device.getDateString(getResponseHandler(req, res));
};

exports.setDate = function (req, res) {
  var time = req.body.time;
  if (checkMissingParams(req, res, {time: time})) {
    req.device.setDate(time, getResponseHandler(req, res));
  }
};

exports.adbInputText = function (req, res) {
  var text = req.body.text;
  if (checkMissingParams(req, res, {text: text})) {
    req.device.adbInputText(text, getResponseHandler(req, res));
  }
};

exports.getVersions = function (req, res) {
  var pkg = req.body.pkg;
  if (checkMissingParams(req, res, {pkg: pkg})) {
    req.device.getVersions(pkg, getResponseHandler(req, res));
  }
};
...
exports.getOwner = function (req, res) {
  var path = req.body.path;
  if (checkMissingParams(req, res, {path: path})) {
    req.device.getOwner(path, getResponseHandler(req, res));
  }
};

exports.setOwner = function (req, res) {
  var path = req.body.path,
      user = req.body.user,
      group = req.body.group;
  if (checkMissingParams(req, res, {path: path, user: user, group: group})) {
    req.device.setOwner(path, user, group, getResponseHandler(req, res));
  }
};
...
exports.cp = function (req, res) {
  ...
  if (checkMissingParams(req, res, {source: source, destination: destination})) {
    req.device.cp(source, destination, getResponseHandler(req, res));
  }
};

exports.startAction = function (req, res) {
  var action = req.body.action;
  var category = req.body.category;
  var data = req.body.data;
  if (checkMissingParams(req, res, {action: action})) {
    req.device.startAction(action, category, data, getResponseHandler(req, res));
  }
};

exports.runAdbShellCommand = function (req, res) {
  var command = req.body.command;
  if (checkMissingParams(req, res, {command: command})) {
    req.device.runAdbShellCommand(command, getResponseHandler(req, res));
  }
};
...
exports.download = function (req, res) {
  var source = req.body.source,
      destination = req.body.destination;
  if (checkMissingParams(req, res, {source: source, destination: destination})) {
    req.device.download(source, destination, getResponseHandler(req, res));
  }
};
...
exports.unzip = function (req, res) {
  var path = req.body.path;
  if (checkMissingParams(req, res, {path: path})) {
    req.device.unzip(path, getResponseHandler(req, res));
  }
};

exports.setOwnerRecursively = function (req, res) {
  var path = req.body.path,
      user = req.body.user,
      group = req.body.group;
  if (checkMissingParams(req, res, {path: path, user: user, group: group})) {
    req.device.setOwnerRecursively(path, user, group, getResponseHandler(req, res));
  }
};
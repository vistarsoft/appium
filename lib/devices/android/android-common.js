...
var logTypesSupported = {
  'logcat' : 'Logs for Android applications on real device and emulators ' +
             'via ADB',
  'logcat-iterative' : 'Logs for Android applications on real device and emulators ' +
             'via ADB, served in chunks with a size limitation'
};
...
androidCommon.setAndroidArgs = function () {
  ...
  this.setArgFromCap("logcatMessageLimit", "logcatMessageLimit");
  this.setArgFromCap("logcatTextBodyLimit", "logcatTextBodyLimit");
  this.setArgFromCap("skipActivityWaiting", "skipActivityWaiting");
  ...
  this.args.appWaitPackage = this.args.androidWaitPackage || this.args.appPackage;
  this.args.skipActivityWaiting = this.args.skipActivityWaiting || false;
  if (!this.args.skipActivityWaiting) {
    this.args.appWaitActivity = this.args.androidWaitActivity || this.args.appActivity;
  }
    ...
    var avdName = this.args.avd.replace('@', ''),
        start = function () {
          this.adb.getRunningAVD(avdName, function (err, runningAVD) {
            ...
            if (this.args.proxy) {
                this.args.avdArgs += " -http-proxy " + this.args.proxy.httpProxy
            }
            this.adb.launchAVD(this.args.avd, this.args.avdArgs, this.args.language, this.args.locale,
                this.args.avdLaunchTimeout, this.args.avdReadyTimeout, cb, 0, this.args.checkSdcard);
          }.bind(this));
        }.bind(this);
    if (this.args.shutdownAvd) {
      this.adb.killAVD(function (err) {
...
androidCommon.installAppForTest = function (cb) {
  ...
  var install = function (installed, remoteApk) {
    if (installed && this.args.fastReset && remoteApk) {
      ...
      var pattern = this.args.appActivity.split('.').slice(0, 3).join('.');
      this.adb.getMatchedPackages(pattern, function (err, packages) {
        if (err) return cb(err);
        logger.debug('Installed packages: ' + packages.join(', '));
        this.adb.uninstallApks(packages, function (err) {
          if (err) return cb(err);
  ...
  var afterSigning = function (err) {
    ...
    this.remoteApkExists(function (err, remoteApk) {
      ...
      this.adb.isAppInstalled(this.args.appPackage, function (err, installed) {
        if (installed) {
          this.adb.getVersions(this.args.appPackage, function (err, versions) {
            this.adb.getVersionFromManifest(this.args.app, function (err, version) {
              var installed = versions.filter(function (v) {
              ...
              logger.debug('Installed versions: ' + stringify(versions));
              logger.debug('Test version: ' + stringify(version));
              install(installed, remoteApk);
            }.bind(this));
          }.bind(this));
        } else {
          install(installed, remoteApk);
...
androidCommon.getLog = function (logType, cb) {
  ...
  } else if (logType === 'logcat-iterative') {
    try {
      logs = this.adb.getLogcatLogsIteratively(this.args.logcatMessageLimit, this.args.logcatTextBodyLimit);
    } catch (e) {
      return cb(e);
    }
  }
...
androidCommon.extractStrings = function (cb, language) {
  logger.debug("Extracting strings for language: " + (language || "default"));
  var outputPath = support.tempDir.path(this.args.appPackage);
  ...
  if (!fs.existsSync(this.args.app)) {
    ...
    this.extractLocalizedStrings(language, outputPath, function (err, stdout, stderr) {
      ...
      var file;
      try {
        logger.debug("Reading strings from converted strings.json");
        var p = path.join(outputPath, stringsJson);
        file = fs.readFileSync(p).toString('utf8');
        this.tempFiles.push(p);
        this.apkStrings = JSON.parse(file);
      } catch (e) {
        var msg = "Could not parse strings from strings.json. Original " +
                  "error: " + e.message;
        logger.debug(msg);
        if (file) {
          logger.debug("Content started with: " + file.slice(0, 300));
        }
        return cb(new Error(msg));
      }
      logger.debug("Setting language to " + (language || "default"));
...
androidCommon.parseJavaVersion = function (stderr) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (new RegExp("java version|openjdk version").test(line)) {
...
androidCommon.stopApp = function (cb, pkg) {
  this.adb.forceStop(pkg || this.args.appPackage, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidCommon.broadcastIntent = function (intent, pkg, receiver, keys, cb) {
  this.adb.broadcastIntent(intent, pkg || this.args.appPackage, receiver, keys, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidCommon.startAction = function (action, category, data, cb) {
  this.adb.startAction(action, category, data, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidCommon.runAdbShellCommand = function (command, cb) {
  this.adb.runAdbShellCommand(command, function (err, lines) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: lines
...
androidCommon.isSoftKeyboardPresent = function (cb) {
  this.adb.isSoftKeyboardPresent(function (err, shown) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: shown
...
androidCommon.clearData = function (pkg, cb) {
  this.adb.clear(pkg || this.args.appPackage, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidCommon.hasRoot = function (cb) {
  this.adb.hasSudo(function (err, rooted) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: rooted
...
androidCommon.getDate = function (cb) {
  this.adb.getDate(function (err, time) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: time
...
androidCommon.getDateString = function (cb) {
  this.adb.getDateString(function (err, date) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: date
...
androidCommon.setDate = function (time, cb) {
  this.adb.setDate(time, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });

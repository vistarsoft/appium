  ...
  this.adb.isScreenLocked(function (err, isLocked) {
    ...
    if (isLocked) {
      ...
      var unlockAndCheck = function () {
        ...
        this.adb.startApp(startOpts, function () {
          this.adb.startApp(startOpts, onStart);
        }.bind(this));
      }.bind(this);
      unlockAndCheck();
    } else {
...
androidController.installApp = function (appPath, replace, cb) {
  var deviceId = this.adb.curDeviceId;
  if (typeof replace === "function") {
    cb = replace;
    replace = false;
  }
  this.adb.install(appPath, replace, function (err) {
    if (err) {
...
androidController.removeFile = function (path, cb) {
  this.adb.rm(path, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.removeFolder = function (path, cb) {
  this.adb.rimraf(path, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.listFiles = function (path, cb) {
  this.adb.ls(path, function (err, files) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: files
...
androidController.listFolder = function (path, cb) {
  this.adb.listFolder(path, function (err, files) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: files
...
androidController.swipeUpHomeButton = function (cb) {
  var that = this;
  this.adb.getNavigationBarRegion(function (err, region) {
    if (err) return cb(err);
    var duration = 2000,
        sleep = 2,
        x = Math.round((region.x2 - region.x1) / 2),
        y = parseInt(region.y1) + Math.round((region.y2 - region.y1) / 2);
    that.adbSwipe(x, y, x - 200, y - 300, duration, sleep, cb);
  });
};

androidController.adbSwipe = function (x1, y1, x2, y2, duration, sleep, cb) {
  this.adb.swipe(x1, y1, x2, y2, duration, sleep, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.adbTap = function (x, y, cb) {
  this.adb.tap(x, y, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.getNavigationBarRegion = function (cb) {
  this.adb.getNavigationBarRegion(function (err, region) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: [region.x1, region.y1, region.x2, region.y2]
    });
  });
};

androidController.replaceApp = function (path, req, cb) {
  var install = function (appPath) {
        this.adb.install(appPath, true, function (err) {
          if (err) return cb(err);
          cb(null, {
            status: status.codes.Success.code
          });
      ...
      sign = function (appPath) {
        if (this.args.noSign) {
          logger.debug('noSign capability set to true, skipping checking and signing of app');
          install(appPath);
        } else {
          this.adb.checkAndSignApk(appPath, this.args.appPackage, function (err) {
            if (err) return cb(err);
            install(appPath);
          });
        }
  ...
  if (path.slice(-3) === 'apk') {
    if (path.indexOf('http') > -1) {
      helpers.downloadFile(path, ".apk", function (err, file) {
        if (err) return cb(err);
        sign(file);
      }.bind(this));
    } else {
    ...
    this.unpackApp(req, function (unpackedAppPath) {
      if (unpackedAppPath === null) return cb('Only a (zipped) app/apk files can be installed using this endpoint');
      sign(unpackedAppPath);
    });
  }
};

androidController.getDisplayMetrics = function (cb) {
  this.adb.getDisplayMetrics(function (err, data) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: data
...
androidController.adbInputText = function (text, cb) {
  this.adb.inputText(text, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.getVersions = function (pkg, cb) {
  this.adb.getVersions(pkg, function (err, data) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: data
...
androidController.getOwner = function (path, cb) {
  this.adb.getOwner(path, function (err, data) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
      , value: data
...
androidController.setOwner = function (path, user, group, cb) {
  this.adb.setOwner(path, user, group, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.cp = function (source, destination, cb) {
  this.adb.cp(source, destination, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.download = function (source, destination, cb) {
  if (destination.slice(-1) === '/') {
    destination += source.split('/').pop();
  }
  helpers.downloadFile(source, ".downloaded", function (err, file) {
    if (err) return cb(err);
    this.adb.push(file, destination, function (err) {
      if (err) return cb(err);
      cb(null, {
        status: status.codes.Success.code
      });
...
androidController.setOwnerRecursively = function (path, user, group, cb) {
  this.adb.setOwnerRecursively(path, user, group, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidController.unzip = function (path, cb) {
  var dir = path.replace('.zip', '');
  this.adb.busybox('mkdir -p ' + dir, function (err) {
    if (err) return cb(err);
    var cmd = 'unzip -o ' + path + ' -d ' + dir;
    this.adb.busybox(cmd, function (err) {
      if (err) return cb(err);
      cb(null, {
        status: status.codes.Success.code
      });
